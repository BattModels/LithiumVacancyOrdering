...
class VacancyOrdering:
    ...
    def generate_atoms(self, x, y, z, atoms_dict, calc, triangular=False, T=0.1, T_max=1505, anneal_steps=10, relax=True): 
        """
        Generates a given NMC composition from scratch using the previous NMC Cluster Expansion code 
        to find the ground state cation arrangment. 
        ...
        atoms_dict : :class:`dict`
            A dictionary of the number of each transition metal in your system (example is 
            atoms_dict = {'Co': 3, 'Ni': 3, 'Mn': 3})
        ...
        triangular : :class:`bool`
            Flag for whether to use a triangular cell. Otherwise, the default cell is rhombohedral
        T : :class:`float`
            Temperature to run the cluster expansion MonteCarlo code to optimize cation position. 
            The default is to try to find the universal ground state at T~0 K
        T_max : :class:`float`
            Maximium temperature for simulated annealing 
        ...
        self.z =z
        self.calc = calc
        if any(atom not in ['Ni', 'Mn', 'Co'] for atom in atoms_dict.keys()):
            sys.exit('Currently only NMC can be modeled') #Expanding to Al is relatively easy
        if triangular:
            N_cations=3*3*x*y*z
        else:
            N_cations=3*x*y*z 
        if np.sum([val for val in atoms_dict.values()]) != N_cations:
            sys.exit('Number of atoms specified in atoms_dict and cell size do not match')

        J=interactions()
        kT=T*8.6173303*10**(-5)
        
        lat=MonteCarlo(x,y,z=z,triangular=triangular,elements=atoms_dict.keys(),directory='/home/ghouchin/layered-oxide-cluster-expansion/')
        if 'Mn' not in atoms_dict.keys():
            atoms_dict['Mn']=0
        if 'Co'not in atoms_dict.keys():
            atoms_dict['Co']=0
        lat.initialize(atoms_dict['Mn'],atoms_dict['Co'],al=0,J=J) 
        T_max=T_max
        T_step = (T_max-5)/float(anneal_steps)
        #Simulated annealing
        for i in range(anneal_steps):
            lat.simulate(100*lat.lat_size,T=T_max-T_step*i)
        #Final relax at temp
        lat.simulate(100*lat.lat_size,T=T)

        #Get the number of Li sites. This is the same as the number of transition metal sites
        #self.num_sites = N_cations
        #return lat.get_atoms()
        self.atoms = lat.get_atoms()
        
        #We then optimize the lattice constants and internal positions twice as this has been shown to be needed when starting from the generic LiNiO2 cifs
        self.atoms.set_calculator(self.calc)
        self.optimize_struct(step=0.01, N_step=7, relax=relax, fmax=.25)
        self.optimize_struct(step=0.01, N_step=7, relax=relax, fmax=.25)
